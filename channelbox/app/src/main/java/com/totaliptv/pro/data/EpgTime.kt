 ...
 *
 * Android already treats naive `yyyy-MM-dd HH:mm:ss` as the device zone
 * ([ZoneId.systemDefault]). This port matches desktop 1.2.11:
 * - Unix timestamps that agree with OS/provider wall time stay UTC instants.
 * - Naive text uses the device/provider zone (never forced UTC).
 * - Local wall stuffed into the unix field is corrected via provider zone.
