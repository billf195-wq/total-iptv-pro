#!/usr/bin/env bash
# Build desktop archives from the current tree (test packages unless asked):
...
# Do not publish these as Latest over a shipping release unless the user says so.
set -euo pipefail
...
WIN_BASE_TAG="${WIN_BASE_TAG:-v1.2.9}"
WIN_BASE_ASSET="${WIN_BASE_ASSET:-TotalIptvPro-Desktop-1.2.9-windows.zip}"
...
[[ -f "$APP_JAR" ]] || { echo "App JAR missing under $APP_DIR/lib/app"; ls -la "$APP_DIR/lib/app" | head; exit 1; }
echo "    App JAR: $(basename "$APP_JAR")"

if [[ -z "$WIN_BASE_ZIP" ]]; then
  for candidate in \
    "/tmp/inspect-128/$WIN_BASE_ASSET" \
    "/tmp/win-base/$WIN_BASE_ASSET" \
    "/tmp/inspect-127/TotalIptvPro-Desktop-1.2.7-windows.zip" \
    "/tmp/inspect-126/TotalIptvPro-Desktop-1.2.6-windows.zip"
  do
    if [[ -f "$candidate" ]]; then
      WIN_BASE_ZIP="$candidate"
      break
    fi
  done
fi
if [[ -z "$WIN_BASE_ZIP" || ! -f "$WIN_BASE_ZIP" ]]; then
  echo "==> Downloading $WIN_BASE_ASSET from $WIN_BASE_TAG as jpackage shell"
  mkdir -p /tmp/win-base
  gh release download "$WIN_BASE_TAG" --repo billf195-wq/total-iptv-pro \
    -p "$WIN_BASE_ASSET" -D /tmp/win-base
  WIN_BASE_ZIP="/tmp/win-base/$WIN_BASE_ASSET"
fi
[[ -f "$WIN_BASE_ZIP" ]] || { echo "Windows base zip missing: $WIN_BASE_ZIP"; exit 1; }
echo "    Windows shell: $WIN_BASE_ZIP"
...
normalized = {x.replace("\\", "/") for x in names}
need = ["runtime/bin/java.exe", "runtime/bin/javaw.exe", "TotalIptvPro.exe"]
missing = [n for n in need if n not in normalized]
if missing:
