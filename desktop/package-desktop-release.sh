...
WIN_BASE_TAG="${WIN_BASE_TAG:-v1.2.7}"
WIN_BASE_ASSET="${WIN_BASE_ASSET:-TotalIptvPro-Desktop-1.2.7-windows.zip}"
...
if [[ -z "$WIN_BASE_ZIP" ]]; then
  for candidate in \
    "/tmp/inspect-127/$WIN_BASE_ASSET" \
    "/tmp/win-base/$WIN_BASE_ASSET" \
    "/tmp/inspect-126/TotalIptvPro-Desktop-1.2.6-windows.zip" \
    "/tmp/inspect-125/TotalIptvPro-Desktop-1.2.5-windows.zip"
  do
    if [[ -f "$candidate" ]]; then
