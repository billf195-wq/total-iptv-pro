...
object M3uKind {
    ...
    fun isVod(url: String, group: String): Boolean {
        if (urlPath(url).contains("/movie/")) return true
        val g = group.trim().lowercase()
        if (g == "movie" || g == "movies" || g == "vod" || g == "films" || g == "film") return true
        if (g.startsWith("movie ") || g.startsWith("movies ") || g.startsWith("vod ") || g.startsWith("vod|") || g.startsWith("movie|")) {
            return true
        }
        val hay = "$url $group".lowercase()
        return hay.contains(".mp4") || hay.contains(".mkv")
    }
