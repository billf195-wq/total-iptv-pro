...
- **Personal DVR (1.2.12):** Record now from Live/Guide, schedule an EPG program, stop, and play/delete from Recordings. Files stay on **this PC** (`%LOCALAPPDATA%\TotalIptvPro\Recordings` on Windows, `~/Videos/TotalIptvPro/Recordings` on Linux). Settings can pick another local folder. Prefers ffmpeg on PATH, else VLC file output, else HLS/.ts download. One recording at a time. Metadata: `recordings.json` next to prefs.
